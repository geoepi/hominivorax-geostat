...
source(here("R/plot_bivariate_response.R"))
```
...
time_index <- make_week_index(study_start, study_end)
# Legacy plotting chunks below use year/week; these aliases retain the shared ISO keys.
time_index$year <- time_index$epiyear
time_index$week <- time_index$epiweek
...
quad_expand <- crossing(integ_df, time_index) %>%
  transmute(Y1 = Y1, Exp = Exp, x = x, y = y,
            year = epiyear, week = epiweek, time_index = time_index)
...
nws_detections_df <- sim_data_df %>%
  mutate(Y1 = 1, # NWS detection was reported
         Exp = NA,
         time_index = match(paste(year, week), paste(time_index$epiyear, time_index$epiweek))) %>% # no exposure (point locations, no area)
  select(Y1, Exp, x, y, year, week, time_index)
```

## Combine Detection Tier
Combine mesh nodes (quadrature) with NWS detections (simulated)
```{r}
tier1_df <- dplyr::bind_rows(nws_detections_df, quad_expand)
