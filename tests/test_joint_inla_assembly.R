...
if (!requireNamespace("INLA", quietly = TRUE)) {
  ...
  stopifnot(isTRUE(first$prediction_compatibility$compatible), all(first$prediction_compatibility$nonfinite_counts == 0L))
  stopifnot(any(first$build_audit$metric == "prediction_compatible"), any(first$build_audit$metric == "prediction_nonfinite_cattle_q"))
  stopifnot(first$build_audit$value[first$build_audit$metric == "prediction_nonfinite_cattle_q"] == "0")
