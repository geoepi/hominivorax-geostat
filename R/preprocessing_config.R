...
read_preprocessing_config <- function(path, repo_root = getwd()) {
  ...
  input_path_names <- setdiff(names(cfg$inputs), "observation_mode")
...
validate_preprocessing_config <- function(cfg, require_inputs = TRUE) {
  ...
  if (isTRUE(require_inputs)) { input_paths <- cfg$inputs[setdiff(names(cfg$inputs), "observation_mode")]; p <- unlist(c(input_paths, cfg$dynamic_covariates, cfg$static_covariates)); p <- p[!vapply(p, file.exists, logical(1))]; if (length(p)) stop("Required input paths are absent: ", paste(p, collapse = ", ")) }
  invisible(TRUE)
}
