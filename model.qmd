...
## Standardized preprocessing output

The model-facing object is now produced by `scripts/run_preprocessing.R`. The compatibility adapter below preserves the legacy names used by the model demonstration while the canonical source remains `model_inputs.rds`.

```{r eval=FALSE}
source(here::here("R/load_preprocessing.R"))
load_preprocessing(here::here())
model_inputs <- readRDS(here::here("outputs/preprocessing/model_inputs.rds"))
tier_1.set <- model_inputs$tier1
tier_2.set <- model_inputs$tier2
grid_combos <- model_inputs$prediction_grid
mesh.dom <- model_inputs$mesh
tier_1.set <- add_legacy_model_fields(tier_1.set)
tier_2.set <- add_legacy_model_fields(tier_2.set)
```
...
### Abundance Stack
```{r eval=FALSE}
locs_t2 <- cbind(tier_2.set$x, tier_2.set$y) # geographic coordinates
